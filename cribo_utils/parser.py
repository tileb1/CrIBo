...
def get_args_parser():
    parser = argparse.ArgumentParser('CrIBo', add_help=False)

    # Model parameters
    parser.add_argument('--image_queries', default=['.'], type=str,
                        help='please specify path to the training data.')  # TODO: delete
    ...
    parser.add_argument('--out_dim_c', default=65536, type=int,
    ...
    parser.add_argument('--warmup_teacher_temp_c', default=0.04, type=float,
                        help="""Initial value for the teacher temperature: 0.04 works well in most cases.
        Try decreasing it if the training loss does not decrease.""")
    parser.add_argument('--teacher_temp_c', default=0.07, type=float, help="""Final value (after linear warmup)
        of the teacher temperature. For most experiments, anything above 0.07 is unstable. We recommend
        starting with the default value of 0.04 and increase this slightly if needed.""")
    ...
    parser.add_argument('--min_lr', type=float, default=0.00001, help="""Target LR at the
    ...
    # CrIBo
    parser.add_argument("--dataset_type", default="imagenet1k", type=str, choices=['imagenet1k'],
                        help="""What dataset to train on.""")
    parser.add_argument("--imagenet1k_path", default="", type=str,
                        help="""Points to the root folder of ImageNet1k. If it is a tar file, it will get untared to --untar_path""")
    parser.add_argument("--coco_path", default=".", type=str,
                        help="""Points to the root folder of coco. If it is a tar file, it will get untared to --untar_path""")
    ...
    parser.add_argument("--pos_alpha", default=(1.0, 1.0), nargs='+', type=float)
    parser.add_argument("--which_features", type=str, default="last", choices=["last"],
                        help="Which features to use", )
    parser.add_argument('--n_tokens', default=32, type=int)
    parser.add_argument("--same_teacher_augmentations", default=False, type=bool_flag)
    ...
    parser.add_argument("--queue_size", default=25000, type=int)
    parser.add_argument('--n_iter', type=int, default=5, help="""How many iterations for centroid initialization.""")
    parser.add_argument("--update_c", type=bool_flag, default=True,
                        help="""Whether to update the marginal during initialisation or not.""")
    parser.add_argument('--normalized_matching', type=bool_flag, default=True)
    return parser
